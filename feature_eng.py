...
def process_amenities():
    ...
    df1.loc[df1['amenities'].str.contains('Amazon Echo|Apple TV|Game console|Netflix|Projector and screen|Smart TV'), 'high_end_electronics'] = 1
    ...
    df1.loc[df1['amenities'].str.contains('Beach view|Beachfront|Lake access|Mountain view|Ski-in/Ski-out|Waterfront'), 'nature_and_views'] = 1
    ...
    return df1[['check_in_24h','air_conditioning','high_end_electronics','nature_and_views','bed_linen','breakfast','tv','coffee_machine',
    'cooking_basics','white_goods','elevator','gym','child_friendly','parking','outdoor_space','host_greeting','hot_tub_sauna_or_pool',
    'internet','long_term_stays','pets_allowed','private_entrance','secure','self_check_in','smoking_allowed','accessible','event_suitable']]
...
new.columns = ['source','host_is_superhost','host_response_rate','host_response_time','host_accptance_rate',
               ...
               'maximum_nights_avg_ntm','number_of_reviews','instant_bookable','reviews_per_month','availability_90']
...
model.fit(df3,review_scores_cleanliness.fillna(mean(review_scores_cleanliness)))
weights = model.coef_
...
# plt.title("Feature Importance for clean score")
# plt.xlabel("Weight")
# plt.show()

df_train = df3[["event_suitable",'pets_allowed','host_is_superhost','internet','host_greeting','self_check_in','child_friendly','nature_and_views','coffee_machine','outdoor_space']]
