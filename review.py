...
def reviews_bag(reviews,bag):
    reviews_bag = {}
    vectorizer = CountVectorizer(vocabulary=bag,analyzer='word')
...
if __name__ == '__main__':
    ...
    #
    # with open('../FinalML/reviews_bag_100.json', 'w') as f:
    ...
    # comment_all = {}
    # for index, value in listing_id.items():
    #     print("translate" + str(index))
    #     comment_all.setdefault(value,[])
    #     words = translate((comments[index]))
    #     comment_all[value].append(words)
    # with open('../FinalML/comment_all.json', 'w') as f:
    #     json.dump(comment_all, f, cls=NpEncoder)

    with open("comment_all.json", 'r', encoding='UTF-8') as f:
        comment_all = json.load(f)

    for keys,values in comment_all.items():
        sentence = ""
        for s in values:
            s = s.replace("\r","")
            sentence = sentence + s
            comment_all[keys] = sentence
    with open('../FinalML/comment_all.json', 'w') as f:
        json.dump(comment_all, f, cls=NpEncoder)
